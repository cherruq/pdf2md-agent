...
`pdf2md-agent` renders each PDF page to an image and feeds it through a CrewAI pipeline of vision agents (extractor → formatter) to produce language-preserving Markdown. Defaults to the `MiniMax-M3` model at `https://api.minimaxi.com/v1`; any OpenAI-compatible vision endpoint works via `OPENAI_BASE_URL`.
...
- Cache control flags use the inverted `--no-cache-<resource>` pattern; resource names (render, text, resized, extract, format) match the on-disk filenames exactly.
