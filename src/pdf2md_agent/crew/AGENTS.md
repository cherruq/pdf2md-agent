...
| `run_extraction_loop` | extraction.py | One page's extract → format loop |
...
- The `_record_text_layer_fallback` kwarg signature (``idx``, ``total``, ``page_number``, ``page_started``, ``artifacts``, ``completion_label``) matches the historical contract tests rely on; new callers can construct a :class:`FallbackRecord` instead.
...
- **NEVER narrow `except BaseException` to `Exception`** in `extraction.py` (per-page retry-exhaustion path) or `runner.py` (`_run_format_only` retry-exhaustion path). Both are intentional for cleanup before re-raise.
...
- `--no-cache-format` short-circuits the entire per-page pipeline when `format.md` is on disk (uses `is_page_complete` in `cache.py`); `--no-cache-extract` triggers `_run_format_only` to re-use cached `extract.txt`.
