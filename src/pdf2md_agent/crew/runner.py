...
def run_pipeline(
    *,
    ...
    for idx, page in enumerate(pages, start=1):
        ...
        available_images = []
        if assets_dir:
            for img_file in pages_dir.glob(f"page_{page.page_number:04d}_img_*"):
                if img_file.is_file():
        ...
        while True:
            ...
            if needs_coverage_check and reflection_attempts < max_reflections:
                ...
                # Count how many characters from native text are found in the markdown
                hit_count = sum(1 for c in set(native_clean) if c in md_clean)
                unique_native = len(set(native_clean))
                coverage = hit_count / unique_native if unique_native > 0 else 1.0
