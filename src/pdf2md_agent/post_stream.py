    ...
    *,
    ...
    for i in range(len(page_list) - 1):
        ...
        prompt = (
            ...
            f"{next_head}\n\n"
            "任务要求：\n"
            "1. 找出它们真正的连续逻辑，如果语义是连续的（如被截断的句子），请无缝拼合并剔除中间的页眉页脚。\n"
            "2. 如果断层切断了 Markdown 表格，请确保合并后的表格语法正确（例如闭合表格行，删除下一页重复的表头）；如果是被切断的列表，请合并为同一层级的列表项。\n"
            "3. 如果它们在语义上是不连续的两个独立段落，请用双换行符 (\\n\\n) 将它们隔开。\n"
            "4. 严禁篡改或总结原文，只能进行拼接和去重操作。\n"
            "5. 你必须输出经过合并处理后的**完整片段**（即包含我提供给你的所有上下文内容，仅去除重复页眉页脚、修复断层）。绝不能省略、缩写上下文，也不能只输出交界处的一句话！\n"
            "6. 严禁输出任何解释性语言（如'这是合并后的结果'）或将内容包裹在 ```markdown ``` 代码块中，直接输出最终文本。"
        )
