...
| `ctx_probe.py` | probe `/v1/models` for `ctx_limit` | called during ConversionConfig assembly; falls through to hardcoded per-model defaults |
...
cli.main
  └─ cmd_convert(args)
       ...
       └─ pipeline.run_unified_conversion(config) (aliased in cli as _run_pipeline for backward test compatibility)
            ...
            ├─ Step 2: crew.runner.run_pipeline (parallel per-page loop; format/extract skip if valid cache)
